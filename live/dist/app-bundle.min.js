...
var App = /** @class */ (function () {
    ...
    App.prototype.send = function (event) {
        ...
        var sxc = window.$2sxc(btn);
        var wrapper = this.findWrapper(btn);
        // clear all alerts
        ...
        this.autoCollectData(wrapper).then(function (data) {
            // data.Recaptcha = recap;
            // submission
            _this.disableInputs(wrapper, true);
            _this.showOneAlert(wrapper, 'msgSending'); // show "sending..."
            var ws = wrapper.data('webservice'); // should be "Form/ProcessForm" or a custom override
            sxc.webApi.post(ws, {}, data, true)
                .success(function () {
                _this.showOneAlert(wrapper, 'msgOk');
                $(btn).hide();
                // wrapper.find('.' + c.clsForm).hide();
            })
                .error(function () {
                _this.showOneAlert(wrapper, 'msgError');
                _this.disableInputs(wrapper, false);
            });
        });
    };
    App.prototype.findWrapper = function (e) {
        return $(e).closest('.' + this.c.clsWrp);
    };
    App.prototype.showOneAlert = function (wrapper, showId) {
        wrapper.find('.alert').hide();
        if (showId !== '') {
            wrapper.find('#' + showId).show();
        }
    };
    App.prototype.disableInputs = function (wrapper, state) {
    ...
    App.prototype.autoCollectData = function (wrapper) {
        var data = {
            Files: []
        };
    ...
    App.prototype.attachFieldValidateOnce = function () {
